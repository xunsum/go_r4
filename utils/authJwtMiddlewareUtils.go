...
func Sign(mode int, id string, username string) (string, error) {
	...
	var token []byte
	var err error
	if mode == res.USER_MODE {
		token, err = jwt.Sign(pl, hs)
	} else if mode == res.ADMIN_MODE {
...
// Verify 验证
func Verify(mode int, content string, token []byte) (*models.LoginToken, error) {
	pl := &models.LoginToken{}
	var err error
	if mode == res.USER_MODE {
...
func Decode(jwt string) (*models.LoginToken, error) {
	var jsonRes models.LoginToken
